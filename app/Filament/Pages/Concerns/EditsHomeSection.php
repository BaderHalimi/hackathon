...
trait EditsHomeSection
{
    ...
    protected function registrationFields(): array
    {
        return [
            ...
            Section::make('تسميات حقول النموذج')->schema(array_map(fn (string $key) => $this->text("fields.{$key}", str($key)->replace('_', ' ')->headline()), ['full_name', 'email', 'phone', 'university', 'major', 'year', 'skills', 'track', 'experience', 'location', 'team_data', 'team_name', 'team_size', 'members', 'add_member', 'project', 'links', 'source']))->columns(2)->collapsed(),
            Section::make('النصوص المساعدة')->schema(array_map(fn (string $key) => $this->text("placeholders.{$key}", "مثال: {$key}"), ['full_name', 'university', 'major', 'skills', 'team_name']))->columns(2)->collapsed(),
            Section::make('خيارات القوائم')->schema([TagsInput::make('locations')->label('أماكن الحضور (الفروع)')->helperText('يختار المشارك أحد هذه الأماكن عند التسجيل — يمكن إضافة مكان واحد أو أكثر.')->columnSpanFull(), $this->text('location_placeholder', 'عبارة اختيار المكان'), TagsInput::make('year_options')->label('السنوات الدراسية'), TagsInput::make('experience_options')->label('مستويات الخبرة'), TagsInput::make('source_options')->label('مصادر معرفة الحدث'), TagsInput::make('team_sizes')->label('أحجام الفرق'), TagsInput::make('role_options')->label('تخصصات أعضاء الفريق'), $this->text('select_placeholder', 'عبارة الاختيار'), $this->text('track_placeholder', 'عبارة اختيار المسار'), $this->text('track_unsure', 'خيار غير محدد')])->columns(2)->collapsed(),
            Section::make('حقول أعضاء الفريق')->schema([$this->text('member_label', 'تسمية العضو'), $this->text('remove_member', 'زر حذف العضو'), $this->text('member_name_placeholder', 'مثال اسم العضو'), $this->text('member_role_label', 'تسمية تخصص العضو'), $this->text('member_skill_label', 'تسمية دور العضو'), $this->text('member_skill_placeholder', 'مثال دور العضو'), $this->text('members_hint', 'ملاحظة الأعضاء')])->columns(2)->collapsed(),
